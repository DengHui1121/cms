...
func MachineUpdate(db *gorm.DB, p Machine) error {
	db.Transaction(func(tx *gorm.DB) error {
						...
						for bk := range p.Parts[k].Points[pk].Bands {
							if p.Parts[k].Points[pk].Bands[bk].Range == "" {
								err = tx.Table("band").
									...
									Pluck("id", &p.Parts[k].Points[pk].Bands[bk].ID).Error
							} else {
								err = tx.Table("band").
									Where("point_uuid=?", p.Parts[k].Points[pk].UUID).
									Where("value=? ", p.Parts[k].Points[pk].Bands[bk].Value).
							...
							if p.Parts[k].Points[pk].Bands[bk].ID != 0 {
								tx.Table("band").Omit(clause.Associations).
									Select("value", "property", "band_range", "floor_level", "floor_std", "floor_desc", "floor_suggest", "upper_level", "upper_std", "upper_desc", "upper_suggest", "rpm_floor", "rpm_upper").Clauses(clause.Locking{Strength: "UPDATE"}).Updates(&p.Parts[k].Points[pk].Bands[bk])
		...
		db.Table("machine").
			Select("name", "type", "fan_version", "tree_version", "desc", "built_time", "band_alert_set", "tree_alert_set", "unit").
...
func CheckData(db *gorm.DB, pdata *Data) error {
	...
	sub := db.Table(table).Where("point_uuid=?", pdata.PointUUID).
		...
		Count(&count)
	if count != 0 {
		err = sub.Select("id", "uuid").Scan(&pdata).Error
	}
	if err != nil {
		return err
	}
...
func InsertData(ddb *gorm.DB, db *gorm.DB, ipport string, pdata Data) error {
	...
	if strings.ToUpper(pdata.Datatype) == "TACH" {
		err = db.Transaction(func(tx *gorm.DB) error {
			if pdata.ID == 0 {
				err = tx.Table("data_rpm_" + fid).Omit(clause.Associations).Create(&pdata).Error
				if err != nil {
					return err
				}
			} else {
